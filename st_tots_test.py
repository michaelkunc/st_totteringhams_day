...
class TOTSTESTS(unittest.TestCase):
	...
	def setUpClass(TOTSTESTS):
		...
		TOTSTESTS.table_soup = st_tots.Soup('http://www.premierleague.com/en-gb/matchday/league-table.html')
		TOTSTESTS.arsenal = st_tots.Team('Arsenal', TOTSTESTS.table_soup)
		TOTSTESTS.spurs = st_tots.Team('Tottenham Hotspur', TOTSTESTS.table_soup)
...
#need to review this test to make sure it's testing something valuable.
	...
	def test_Team__init__games_played(self):
		self.assertTrue(0 < TOTSTESTS.arsenal.games_played < TOTSTESTS.games_per_season)
...
#this is a slightly gnarley test case.
	def test_Team_get_team_data(self):
		self.assertEqual('<td class="col-pos">3</td>', str(TOTSTESTS.arsenal.get_team_data(TOTSTESTS.table_soup, 'td', "Arsenal")[0]))


	def test_Team_available_points(self):
		self.assertTrue(0 < TOTSTESTS.arsenal.available_points(TOTSTESTS.arsenal.games_played) < TOTSTESTS.max_points)
	...
	def test_end_of_season(self):
		self.assertFalse(st_tots.end_of_season(TOTSTESTS.arsenal, TOTSTESTS.spurs))


	def test_simulate_remaining_season(self):
		st_tots.simulate_remaining_season(TOTSTESTS.arsenal, TOTSTESTS.spurs)
		self.assertEqual( st_tots.Messages.st_tots_message(), st_tots.simulate_remaining_season(TOTSTESTS.arsenal, TOTSTESTS.spurs))
