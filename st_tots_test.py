...
class ST_TOTSTESTS(unittest.TestCase):
	...
	def setUpClass(ST_TOTSTESTS):
		...
		ST_TOTSTESTS.table_soup = st_tots.Soup('http://www.premierleague.com/en-gb/matchday/league-table.html')
		ST_TOTSTESTS.arsenal = st_tots.Team('Arsenal', ST_TOTSTESTS.table_soup)
		ST_TOTSTESTS.spurs = st_tots.Team('Tottenham Hotspur', ST_TOTSTESTS.table_soup)

#need to review this test to make sure it's testing something valuable.
	def test_init_Soup(self):
		class_name = str(type(ST_TOTSTESTS.table_soup))
...
#this is a slightly gnarley test case.
	def test_Team_get_team_data(self):
		self.assertEqual('<td class="col-pos">3</td>', str(ST_TOTSTESTS.arsenal.get_team_data(ST_TOTSTESTS.table_soup, 'td', "Arsenal")[0]))
	...
	def test_check_for_st_tots_achieved(self):
		...
		arsenal = st_tots.Team('Arsenal', ST_TOTSTESTS.table_soup)
		arsenal.points = 100
		spurs = st_tots.Team('Tottenham Hotspur', ST_TOTSTESTS.table_soup)
		spurs.points = 0
